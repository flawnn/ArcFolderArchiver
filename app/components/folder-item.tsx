  ...
  return (
    <div>
      <button
        onClick={handleClick}
        className={cn(
          "w-full flex items-center gap-3 p-4 bg-white/20 backdrop-blur-sm rounded-2xl",
      ...
      >
        ...
        <span className="flex-1 text-base">{folder.name}</span>

        {/* Chevron on the right for folders with children */}
        {isFolder && hasChildren ? (
          <div className="w-4 h-4 flex-shrink-0 ml-3 opacity-80">
            {isExpanded ? (
